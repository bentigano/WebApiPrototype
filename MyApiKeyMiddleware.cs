namespace WebApiPrototype
{
    ...
    /// If not, return a 401 and a response of "Invalid or missing API Key supplied in header X-Api-Key.".
    /// </summary>
    public class MyApiKeyMiddleware
    {
        ...
        public async Task InvokeAsync(HttpContext context)
        {
            ...
            if (!context.Request.Headers.TryGetValue(API_KEY_HEADER_NAME, out var receivedApiKey) || !_validApiKeys.Contains(receivedApiKey.ToString()))
            {
                ...
                await context.Response.WriteAsync($"Invalid or missing API Key supplied in header {API_KEY_HEADER_NAME}.");
                return;
            }
